...
module.exports = async (req, res) => {
  ...
  try {
    await resend.emails.send({
      ...
      html:    buildEmail({ firstName, intro: resourceData.intro, downloadUrl, subject: resourceData.subject }),
    });
...
function buildEmail({ firstName, intro, downloadUrl, subject }) {
  return `<!DOCTYPE html>
<html>
...
<body style="margin:0;padding:0;background:#F5F8FA;font-family:Arial,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:#ffffff;border-radius:10px;overflow:hidden;box-shadow:0 2px 16px rgba(27,46,75,0.09);">
    ...
    <div style="background:#1B2E4B;padding:28px 32px;">
      <p style="margin:0;color:#2ABFBF;font-size:1.1rem;font-style:italic;font-weight:400;">Blue Mountain Bookkeeping</p>
    </div>
    <!-- Body -->
    <div style="padding:32px;">
      <h1 style="margin:0 0 12px;font-size:1.3rem;color:#1B2E4B;">Hi ${firstName}! Your download is ready.</h1>
      <p style="margin:0 0 20px;font-size:0.92rem;color:#4A6070;line-height:1.7;">${intro}</p>
      ...
      <p style="margin:0 0 8px;font-size:0.88rem;color:#4A6070;line-height:1.7;">
        While you're here — if you're ready to go deeper, check out our affordable bookkeeping tools designed to save you hours every month.
      </p>
      <a href="${process.env.SITE_URL}/#tools" style="font-size:0.88rem;color:#1A7A7A;">Browse Bookkeeping Tools →</a>
